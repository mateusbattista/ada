...
interface SelectProps extends React.HTMLAttributes<HTMLSelectElement>, IMtProps {
    ...
    /** valor do id da option. */
    valueID?: string | string[] | number | number[];
    /** Options do select. */
...
const SelectOptionID = React.forwardRef<HTMLSelectElement, SelectProps>(
    ({ className, children, id, label, options, value, onChange = () => {/* */ }, placeholder, type = 'single', selectAllText = 'Selecionar todos', disabled = false, ...props }, ref) => {
        ...
        const handleChangeValue = useCallback((event: React.FormEvent<HTMLInputElement>) => {
            const option = options.find(opt => String(opt.value) === event.currentTarget.value);
            const valueObj = {
                value: event.currentTarget.value,
                id: option?.id // Aqui você pode acessar o id da opção selecionada, se existir
            };
            setCurrentValue(valueObj.value);
            setExpanded(false);
            onChange(valueObj);
        }, [onChange, options, setCurrentValue, setExpanded]);
        ...
        const handleChangeValueMultiple = useCallback((event: React.FormEvent<HTMLInputElement>) => {
            ...
            if (event.currentTarget.checked) {
                setCurrentValue((oldValues: any) => {
                    const newValues = [...oldValues, valueObj];
                    onChange(newValues);
                    return newValues;
                });
            } else {
                setCurrentValue((oldValues: any) => {
                    const newValues = oldValues.filter((val: any) => val.value !== valueObj.value);
                    onChange(newValues);
                    return newValues;
                });
            }
        ...
        return (
            ...
            >
                <div ref={refInputWrapper} className="br-input" style={{
                    maxWidth: '100%'
                }}>
...
SelectOptionID.displayName = 'SelectOptionID';

export default memo(SelectOptionID);
