...
const Table = React.forwardRef<HTMLDivElement, TableProps>(
    ({
        ...
        useEffect(() => {
            if (currentEndpoint && atualizando) {
                setAtualizando(true);
                ...
                <table >
                                        ...
                                        <td key={index}>
                                            {buttons.map((button) => (
                                                <>
                                                    {(button.type === 'link') ? (
                                                        <a
                                                            id={linha[Object.keys(linha)[0]]}
                                                            href={button.href || '#'}
                                                            className={button.classeNome}
                                                            onClick={button.onClick}
                                                        >
                                                            {button.icon && ( // Verifica se button.icon não é nulo
                                                                <i className={`${button.icon.classeNome}`} aria-hidden={button.icon.ariaHidden} />
                                                            )}
                                                        </a>
                                                    ) : (
                                                        <button
                                                        ...
                                                        >
                                                            {button.icon && (
                                                                <div id={linha[Object.keys(linha)[0]]}
                                                                    data-dados={Object.entries(linha).map(([key, value]) => `${key}: ${value}`).join(', ')}
                                                                    onClick={button.onClick}
                                                                    className={`${button.icon.classeNome}`}
                                                                    aria-hidden={button.icon.ariaHidden} />
                                                            )}
                                                        </button>
                                                    )}
                                                </>
                    ...
                    {headers && tableData && (headers as IHeader[])[0].label &&
                        <tbody>
                            {tableData.map((linha, index) => (
                                <tr key={index}>
                                    {headers && linha && buttons !== undefined && (
                                        <td key={index}>
                                            {buttons.map((button) => (
                                                <>
                                                    {(button.type === 'link') ? (
