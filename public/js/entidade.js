...
function appendMeta() {
    const tecnologia = $("#id_tecnologia").val();
    const quantidade = $("#id_quantidade").val();
    const valor_unitario = $("#id_valor_unitario").val();
    const familias_atendidas = $("#id_familias_atendidas").val();
    const data_inicio_meta = $("#id_data_inicio_meta").val();
    const data_fim_meta = $("#id_data_fim_meta").val();


    if (tecnologia == '' || quantidade == '' || valor_unitario == '' || familias_atendidas == '' || data_inicio_meta == '' || data_fim_meta == '') {
        const elemento = `
                                    <div class="br-message danger" id="box-metas" role="alert">
                                        <div class="icon"><i class="fas fa-times-circle fa-lg"
                                                             aria-hidden="true"></i>
                                        </div>
                                        <div class="content"><span
                                                class="message-title">Erro ao inserir meta.</span><span
                                                class="message-body"> TODOS OS CAMPOS SÃO OBRIGATÓRIOS</span>
                                        </div>
                                        <div class="close">
                                            <button class="br-button circle small" type="button"
...
`
        ...
        const elemento = `
       <tr>
           <td data-th="Tecnologia">${tecnologia}</td>
           <input type="hidden" name="tecnologia" value="${tecnologia}">
           <td data-th="Quantidade">${quantidade}</td>
           <input type="hidden" name="quantidade" value="${quantidade}">
           <td data-th="Valor">${valor_unitario}</td>
           <input type="hidden" name="valor_unitario" value="${valor_unitario}">
           <td data-th="Nº de Famílias">${familias_atendidas}</td>
           <input type="hidden" name="familias_atendidas" value="${familias_atendidas}">
           <td data-th="Data Inicial da Meta">${data_inicio_meta}</td>
           <input type="hidden" name="data_inicio_meta" value="${data_inicio_meta}">
           <td data-th="Data Final da Meta">${data_fim_meta}</td>
           <input type="hidden" name="data_fim_meta" value="${data_fim_meta}">
           <td data-th="Ação"> <button class="br-button danger active mr-3 btnDelete" type="button">Remover</button></td>
       </tr>
`
        $("#tabelaMetas").append(elemento);

        $("#tabelaMetas").on('click', '.btnDelete', function () {
            $(this).closest('tr').remove();
        })
    }
}